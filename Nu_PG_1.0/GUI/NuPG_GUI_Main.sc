...
NuPG_GUI_Main {
	...
	build {|recordTasks, playbackTasks, colorScheme = 0|
		...
		var labelNames = [
			"_TRIGGER FREQUENCY",
			"_GRAIN FREQUENCY",
			"_ENVELOPE MULTIPLICATION",
			"_PAN",
			"_AMP"
		];
		//slider record inits
		//var sliderPlayer = NuPG_SliderPalyer;
		//var sliderDataArray = sliderPlayer.sliderArray;
		//var sliderRecordTasks = sliderPlayer.recordTask(data, sliderDataArray);
		//var sliderPlaybackTasks = sliderPlayer.playbackTask(data, sliderDataArray);

		window = Window.new("MAIN",
			Rect.fromArray(defs.nuPGDimensions[2]), resizable: false);
		...
		sliders = 3.collect{5.collect{}};
		numberBoxes = 3.collect{5.collect{}};
		recButtons = 3.collect{5.collect{}};
